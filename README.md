...
The standalone OpenCode TUI bundle deliberately preserves whitespace around external static imports. OpenCode 1.18.18 supplies OpenTUI and Solid through OpenTUI 0.4.5's runtime-module bridge, whose scanner cannot rewrite whitespace-minified `from"package"` syntax. `npm run check:remote-bundles` proves byte-for-byte regeneration, and `npm run test:opencode-tui` imports the copied bundle through that real bridge before checking the `Workflows` sidebar and dashboard, `Open Workflows Dashboard` command-palette reachability in modal mode, the base-only `<leader>w` shortcut, and the flat SDK v2 `sessionID` request shape.
