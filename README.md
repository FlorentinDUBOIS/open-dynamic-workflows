...
For release and support checks, run `npm run smoke:hosts`. It creates a temporary full install, starts a temporary daemon, parses `odw-daemon doctor all --json` so every named adapter is checked explicitly (including zcode), and probes installed host CLIs. Missing proprietary hosts are reported as skipped instead of faking coverage.
